...
export default function AudioLibraryScreen({ navigation }) {
  ...
  const loadLibrary = async () => {
    ...
    // Filtramos solo los audiolibros
    const audiobooks = library.filter(item => item.isAudio);
    setBooks(audiobooks);
    setLoading(false);
  };
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (allFound.length === 0) {
        Alert.alert('Escaneo Finalizado', 'No se encontraron nuevos audiolibros en las carpetas comunes.');
      } else {
        let addedCount = 0;
      ...
      <View style={styles.cardContent}>
        {item.coverUrl ? (
          ...
          <View style={[styles.thumbnail, { backgroundColor: '#333', justifyContent: 'center', alignItems: 'center' }]}>
            <IconButton icon="headphones" size={30} iconColor="#666" />
          </View>
        )}
        <View style={styles.bookInfo}>
          ...
          <Text variant="labelSmall" style={{ marginTop: 4, opacity: 0.6, color: '#FF9800' }}>
            Audiolibro • {new Date(item.dateAdded).toLocaleDateString()}
          </Text>
        </View>
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      {loading ? (
        ...
        <View style={styles.center}>
          <IconButton icon="headphones-off" size={80} iconColor="#333" />
          <Text variant="headlineSmall" style={{ color: '#666', textAlign: 'center' }}>
            {searchQuery ? 'No se encontraron audiolibros' : 'No tienes audiolibros'}
          </Text>
          {!searchQuery && (
            <Text variant="bodyMedium" style={{ color: '#444', marginTop: 10, textAlign: 'center' }}>
              Los audiolibros que descargues aparecerán aquí.
            </Text>
          )}
        </View>
      ) : (
...
const styles = StyleSheet.create({
  ...
  fab: {
    ...
    right: 0,
    bottom: 0,
    backgroundColor: '#FF9800' // Orange for Audio
  }
});
